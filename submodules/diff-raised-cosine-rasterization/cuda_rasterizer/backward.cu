	...
	float* __restrict__ dL_dcolors)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float ellipse = (con_o.x * d.x * d.x + con_o.z * d.y * d.y + 2.f * con_o.y * d.x * d.y);
			if (ellipse < 0 || ellipse > 6.25f) // limit set on 2D ellipse to restrict to one pulse
				continue;
			float ellipse_root = sqrt(ellipse); // dM
			float ellipseFactor = M_PI *ellipse_root / 2.5f;
			
			// if (ellipseFactor > 0.0f)
			// 	continue;

			// const float G = exp(ellipseFactor);
			
			const float Cosine = .5f + .5f* cos(ellipseFactor);
			...
			const float sindL_dCosine = sin(ellipseFactor)* dL_dCosine;
			const float factor1 = - M_PI * sindL_dCosine /5.f / ellipse_root;
			const float factor2 = factor1 * 0.5 ;
			const float dL_ddelx = ( con_o.x * d.x + con_o.y * d.y)* factor1;
			const float dL_ddely = ( con_o.z * d.y + con_o.y * d.x)* factor1;
			...
			atomicAdd(&dL_dconic2D[global_id].x, d.x * d.x * factor2);
			atomicAdd(&dL_dconic2D[global_id].y, d.x * d.y * factor2);
			atomicAdd(&dL_dconic2D[global_id].w, d.y * d.y * factor2);
